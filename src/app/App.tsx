...
const hashPage=()=>{const params=hashParams();if(params.get('type')==='recovery')return'reset-password';const raw=location.hash.replace('#','');return raw.startsWith('access_token=')?'dashboard':raw.split('?')[0]||'dashboard'}
export function App(){
  const {tr,dir}=useI18n();const {user,profile,loading,refresh}=useAuth()
  const [inviteFlow,setInviteFlow]=useState(()=>hashParams().get('type')==='invite')
  ...
  useEffect(()=>{const handler=()=>{const params=hashParams();if(params.get('type')==='invite'){setInviteFlow(true);return}if(params.get('type')==='recovery'){setPage('reset-password');return}const raw=location.hash.replace('#','');if(!raw.startsWith('access_token='))setPage(raw.split('?')[0]||'dashboard')};addEventListener('hashchange',handler);return()=>removeEventListener('hashchange',handler)},[])
  ...
  const finishPasswordReset=()=>{history.replaceState(null,'',`${location.pathname}#dashboard`);setPage('dashboard')}
  ...
  if(page==='reset-password')return <AuthScreen mode="reset-password" onDone={finishPasswordReset}/>
