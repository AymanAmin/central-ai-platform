...
export function App(){
  ...
  const initialPage=()=>{const raw=location.hash.replace('#','');return raw.startsWith('access_token=')?'dashboard':raw||'dashboard'}
  const [page,setPage]=useState(initialPage);const [bootstrapPossible,setBootstrapPossible]=useState(false)
  useEffect(()=>{const handler=()=>{const params=new URLSearchParams(location.hash.slice(1));if(params.get('type')==='invite'){setInviteFlow(true);return}const raw=location.hash.replace('#','');if(!raw.startsWith('access_token='))setPage(raw||'dashboard')};addEventListener('hashchange',handler);return()=>removeEventListener('hashchange',handler)},[])
  ...
  useEffect(()=>{if(profile&&!inviteFlow&&page!=='reset-password'&&!canAccessPage(profile.role,page)){history.replaceState(null,'','#dashboard');setPage('dashboard')}},[page,profile,inviteFlow])
