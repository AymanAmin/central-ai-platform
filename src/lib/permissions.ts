...
const rolePages:Record<AppRole,ReadonlySet<PageKey>>={
  ...
  ORGANIZATION_ADMIN:new Set<PageKey>([
    'dashboard','users','api-clients','integration','web-widgets','knowledge','playground','ai-settings','prompts','tools',
    'customers','conversations','chat-test','handoff','usage','audit',
  ]),
  KNOWLEDGE_MANAGER:new Set<PageKey>(['dashboard','knowledge','playground']),
  SUPPORT_AGENT:new Set<PageKey>(['dashboard','customers','conversations','chat-test','handoff']),
