...
export async function adminApi<T>(body:Record<string,unknown>):Promise<T>{
  ...
  const response=await fetch(`${functionsBaseUrl}/admin-api`,{method:'POST',headers:{authorization:`Bearer ${session.access_token}`,'content-type':'application/json'},body:JSON.stringify(body)})
