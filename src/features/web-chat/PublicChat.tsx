...
import { loadWidgetConfig,loadWidgetDirectory,sendWidgetMessage,sendWidgetVoice,syncWidgetConversation,widgetSession,type WidgetDirectoryItem,type WidgetHistoryMessage,type WidgetPublicConfig,type WidgetSession } from './chatClient'

type Lang='ar'|'en'
type ChatItem={id:string;role:'assistant'|'user';text:string;actions?:Array<Record<string,unknown>>;source?:string;agentName?:string|null;voiceDerived?:boolean;audioUrl?:string|null;audioKind?:'audio'|'tts'|null;audioStored?:boolean;audioDurationMs?:number|null;voiceName?:string|null;audioLanguage?:'ar'|'en'|null}
...
const fromHistory=(items:WidgetHistoryMessage[]):ChatItem[]=>items.map(item=>({id:item.id,role:item.role,text:item.text,actions:item.actions,source:item.source,agentName:item.agentName,voiceDerived:item.voiceDerived,audioUrl:item.audioUrl,audioKind:item.audioKind,audioStored:item.audioStored,audioDurationMs:item.audioDurationMs,voiceName:item.voiceName,audioLanguage:item.audioLanguage}))
...
export function PublicChat(){
  ...
  const submitVoice=async(audio:Blob,durationMs:number)=>{
    ...
    setBusy(true);setError('');setMessages(items=>[...items,{id:pendingId,role:'user',voiceDerived:true,text:label(language,'جارٍ تحويل الرسالة الصوتية إلى نص…','Transcribing voice message…')}])
    try{
      ...
      try{await sync(config,current)}catch{
        if(response.transcript)setMessages(items=>items.map(item=>item.id===pendingId?{...item,text:response.transcript!,voiceDerived:true}:item))
        if(response.answer)setMessages(items=>[...items,{id:`reply-${crypto.randomUUID()}`,role:'assistant',text:response.answer,actions:response.actions}])
      }
  ...
  return <div className="public-chat-page" dir={language==='ar'?'rtl':'ltr'}>
    ...
    {!started&&<main className="public-chat-entry">
      ...
      <section className="public-chat-entry-card">
        ...
        {config&&<div className="public-chat-agent-card" style={{'--chat-accent':config.primaryColor} as React.CSSProperties}><span className="public-chat-agent-avatar">✦</span><div><strong>{title}</strong><small>{orgName}{config.agentName?` · ${config.agentName}`:''}{config.voiceEnabled?` · ${label(language,'صوت','Voice')}`:''}{config.voiceReplyMode!=='text_only'?` · ${label(language,'رد صوتي','Voice replies')}`:''}</small></div><span className="public-chat-live-dot">{label(language,'متصل','Online')}</span></div>}
    ...
    {started&&config&&<main className="public-chat-shell" style={{'--chat-accent':config.primaryColor} as React.CSSProperties}>
      ...
      <section className="public-chat-messages" aria-live="polite">{messages.map(item=><article key={item.id} className={`public-chat-message ${item.role}`}>
        {item.voiceDerived&&<span className="public-chat-voice-tag">🎙 {label(language,'مُفرّغ من رسالة صوتية','Transcribed from voice')}</span>}
        <div>{item.voiceDerived?cleanLegacyVoiceLabel(item.text):item.text}</div>
        {item.audioUrl&&<div className="public-chat-audio"><div><strong>{item.audioKind==='tts'?label(language,'رد صوتي','Voice reply'):label(language,'الرسالة الصوتية','Voice message')}</strong>{item.audioKind==='tts'&&item.audioLanguage==='ar'&&<small>{label(language,'إلقاء عربي بلهجة سعودية','Saudi Arabic delivery')}</small>}{item.voiceName&&item.audioKind==='tts'&&<small>{item.voiceName}</small>}</div><audio controls preload="none" src={item.audioUrl}/></div>}
      ...
      </article>)}{messages.length===1&&suggestions.length>0&&<div className="public-chat-suggestions">{suggestions.map(suggestion=><button key={suggestion} onClick={()=>void send(suggestion)}>{suggestion}</button>)}</div>}{busy&&<div className="public-chat-typing" aria-label={label(language,'جارٍ إرسال الرسالة','Sending message')}><span/><span/><span/></div>}{error&&<div className="public-chat-error" role="alert">{error}</div>}<div ref={endRef}/></section>
      ...
      <footer className="public-chat-footer">Central AI · {config.voiceEnabled?label(language,`نص وصوت آمن · حتى ${config.maxVoiceSeconds} ثانية`,`Secure text & voice · up to ${config.maxVoiceSeconds}s`):label(language,'محادثة آمنة مرتبطة بالجهة','Secure organization-scoped chat')}{config.voiceReplyMode==='voice_on_voice'?label(language,' · الرد على الصوت بصوت',' · voice replies mirror voice input'):config.voiceReplyMode==='always_voice'?label(language,' · الرد الصوتي مفعّل',' · voice replies enabled'):''}</footer>
    </main>}
  </div>
}
