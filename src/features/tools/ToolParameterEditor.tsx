...
export function ToolParameterEditor({value,onChange}:{value:ToolParameter[];onChange:(next:ToolParameter[])=>void}){
  ...
  return <section className="tool-parameter-editor span-2">
    <div className="tool-parameter-head">
      <div><strong>{tr('متغيرات نقطة الاتصال','Endpoint variables')}</strong><p>{tr('عرّف البيانات التي يحتاجها الـ endpoint. الوكيل سيطلب القيم الناقصة من العميل قبل تنفيذ الأداة.','Define the values the endpoint needs. The agent asks the customer for missing values before executing the tool.')}</p></div>
      <button type="button" className="small ghost" onClick={add} disabled={value.length>=12}>+ {tr('إضافة متغير','Add variable')}</button>
    </div>
    {value.length===0?<div className="tool-parameter-empty">{tr('لا توجد متغيرات. ستعمل الأداة بدون بيانات من العميل.','No variables yet. The tool will run without customer-provided input.')}</div>:<div className="tool-parameter-list">{value.map((row,index)=><div className="tool-parameter-row" key={`${row.key}-${index}`}>
    ...
    <FieldHint>{tr('GET يرسل القيم كـ query parameters. POST يرسلها داخل JSON body. لا يستطيع النموذج إضافة مفاتيح غير معرفة هنا عند وجود متغيرات معرفة.','GET sends values as query parameters. POST sends them in the JSON body. When variables are defined, the model cannot add undeclared keys.')}</FieldHint>
  </section>
}
