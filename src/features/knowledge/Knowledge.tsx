...
export function Knowledge({ profile }: { profile: Profile }) {
  ...
  return <>
    <PageHeader
      ...
      description={tr('قواعد المعرفة والمستندات والروابط والأسئلة الشائعة والنصوص اليدوية بالعربية والإنجليزية.', 'Knowledge bases, documents, web pages, FAQs, and manual text in Arabic and English.')}
    />

    <Card>
      <div className="grid-form">
        {profile.role === 'SUPER_ADMIN' && <select value={organizationId} onChange={e => { setOrganizationId(e.target.value); setSelectedKb('') }}>
    ...
    <div className="two-col">
      <Card>
        ...
        <form className="stack" onSubmit={createKb}>
          <input required placeholder={tr('الاسم', 'Name')} value={kbName} onChange={e => setKbName(e.target.value)} />
          <input required placeholder="CODE" value={kbCode} onChange={e => setKbCode(e.target.value)} />
          <button disabled={!organizationId}>{tr('إنشاء', 'Create')}</button>
        </form>
      </Card>

      <Card>
        <h2>{tr('إضافة مصدر', 'Add source')}</h2>
        <div className="stack">
          ...
          <h3>{tr('رابط صفحة ويب', 'Web page URL')}</h3>
          <form className="stack" onSubmit={addUrl}>
            <input placeholder={tr('عنوان اختياري للمصدر', 'Optional source title')} value={urlTitle} onChange={e => setUrlTitle(e.target.value)} />
            ...
            <small>{tr('يتم جلب صفحة واحدة فقط. الروابط الداخلية والمحلية محظورة أمنيًا، والحد الأقصى للصفحة 2MB.', 'One page only. Private/internal addresses are blocked for security and the page limit is 2MB.')}</small>
            <button disabled={!selectedKb}>{tr('إضافة الرابط', 'Add URL')}</button>
          </form>

          <h3>{tr('نص يدوي', 'Manual text')}</h3>
          <form className="stack" onSubmit={addManual}>
            <input required placeholder={tr('عنوان النص اليدوي', 'Manual text title')} value={manualTitle} onChange={e => setManualTitle(e.target.value)} />
            <textarea required rows={4} value={manualText} onChange={e => setManualText(e.target.value)} placeholder={tr('النص', 'Text')} />
            <button disabled={!selectedKb}>{tr('إضافة نص يدوي', 'Add manual text')}</button>
          </form>
        </div>
      </Card>
    </div>
    ...
    <Card>
      ...
      <form className="grid-form" onSubmit={addFaq}>
        <input required placeholder={tr('السؤال', 'Question')} value={faqQ} onChange={e => setFaqQ(e.target.value)} />
        <input required placeholder={tr('الإجابة', 'Answer')} value={faqA} onChange={e => setFaqA(e.target.value)} />
        <button disabled={!selectedKb}>{tr('إضافة', 'Add')}</button>
      </form>
    </Card>

    <Card>
      <h2>{tr('المستندات والمصادر', 'Documents and sources')}</h2>
      {visibleDocs.length === 0 ? <Empty>{tr('لا توجد مستندات.', 'No documents found.')}</Empty> : <table>
        <thead><tr>
          <th>{tr('العنوان', 'Title')}</th>
          <th>{tr('النوع', 'Type')}</th>
          <th>{tr('اللغة', 'Language')}</th>
          <th>{tr('الحالة', 'Status')}</th>
          <th>{tr('المعالجة', 'Processing')}</th>
        </tr></thead>
        <tbody>{visibleDocs.map(d => <tr key={d.id}>
          ...
          <td>{valueLabel(d.processing_status)}{d.processing_error && <small className="error-text"> — {d.processing_error}</small>}</td>
        </tr>)}</tbody>
      </table>}
    </Card>
  </>
